...
· **password 字段掩码回传**（v0.4 验收发现：密码以明文进入信封与会话日志；
  可选方案为信封中替换为 `"***"` 或文档明示禁收真实密码）
· **渲染错误边界**（v0.4 首版 Form 把 spec 原对象塞给 React 导致整卡消失，
  顶层降级未兜住；可加 React ErrorBoundary 兜底为提示卡而非空白）
