...
export const GENUI_GUIDANCE = '本机已安装 dsh-genui 插件（聊天生成式 UI）：模型可调用 ui_card 工具，把结构化/可视化内容（仪表盘、指标、对比表、时间线、进度、图表、告警）以 JSON 组件树传入，聊天界面会渲染为真正的界面卡片而非 markdown。支持组件：card/row/stat(带涨跌)/table(可排序，filter:true 开筛选)/progress/timeline/chart(kind:bar柱状/line折线/pie饼图/horizontal横条,color 可覆色)/kv/badges/alert(四色)/code/collapse(折叠面板)/tabs(选项卡)/buttons/form(表单：包裹 input/select/textarea，字段需唯一 name)/text，stat 可带 trend 数组渲染 sparkline 迷你趋势，可任意嵌套，值必须是纯 JSON。交互能力：buttons 的 items 可传 {label,action,confirm}，用户点击（默认需二次确认）会以用户消息把事件发回对话流；表单提交把全部字段打包进信封 payload 一次回传，省额度。collapse/tabs/表格排序筛选为本地交互。事件格式：收到「[ui_card 事件] {json}」开头的用户消息时，解析其 JSON 信封（cardId 定位来源卡片、action 为请求动作、payload 为表单字段集合），按对话上下文执行。工具返回的 warnings 列出 spec 中将被丢弃/截断的字段路径，请据此修正后再渲染。回答包含结构化或视觉数据、或用户要求「卡片/面板/仪表盘/可视化」时优先使用 ui_card 展示。'

/** The component vocabulary, shared by the tool description. */
const NODES = 'card{title,children}, row{children}, stat{label,value,delta?,trend?:numbers}, stat-row{children}, table{columns,rows,filter?}, progress{label,value}, timeline{items:[{title,desc}]}, chart{kind:bar|line|pie|horizontal,items:[{label,value,color?}],color?}, kv{items:[{key,value}]}, badges{items:strings}, alert{tone:info|ok|warn|err,text}, code{text}, collapse{title,open,children}, tabs{items:[{label,children}]}, buttons{items:strings|{label,action,confirm}}, form{action,submitLabel,confirm?,children}, input{name,label,value?,placeholder?,password?}, select{name,label,options:strings}, textarea{name,label,value?,placeholder?}, text{text}'
...
function precheck(spec) {
  ...
  function walk(node, path, depth) {
    ...
    if (type === 'chart' && Array.isArray(node.items)) {
      if (node.items.length > 24) notes.push('chart.items: ' + node.items.length + ' 项超出上限 24，将截断')
      const kind = typeof node.kind === 'string' ? node.kind : ''
      if (kind && !['bar', 'line', 'pie', 'horizontal'].includes(kind)) {
        notes.push('chart.kind: 「' + kind + '」不受支持，可用 bar|line|pie|horizontal，将回退为 bar')
      }
      if (kind === 'pie' && node.items.length > 8) {
        notes.push('chart(pie): ' + node.items.length + ' 项偏多，饼图建议 ≤8 项以保持可读')
      }
    }
    if (type === 'table') {
