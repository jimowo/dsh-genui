	...
	factory: (require) => {
		...
		function Buttons({ items, env }) {
			...
			async function fire(index) {
				...
				try {
					await send('[ui_card 事件] 用户点击了按钮「' + b.label + '」'
		 ...
		 *  carries the per-card send function for interactive event buttons. */
		function renderNode(node, depth, env) {
			...
			if (type === 'badges') {
				const items = Array.isArray(node.items) ? node.items.map(text).slice(0, 30) : []
			...
			if (type === 'kv') {
				...
				return createElement('div', { className: 'genui-kv' }, items.map((item, index) => {
					...
					return createElement(Fragment, { key: 'kv' + index },
						createElement('div', { className: 'genui-kv-k' }, text(item.key)),
						createElement('div', null, text(item.value)))
				}))
			}
			if (type === 'code') {
		...
		function GenUICard(props) {
			...
			return createElement('div', { className: 'genui-root' }, renderNode(spec, 0, { send: makeSend(props) }))
		}
