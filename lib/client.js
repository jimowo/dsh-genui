		...
		function makeSend(props) {
			return async function send(text) {
				...
				if (sid && rootCtx && rootCtx.sessions) {
					try {
						const scoped = rootCtx.sessions.scope(sid)
						// Property access itself throws when the scope lacks the
						// inject declaration ("cannot get property without inject").
						conversation = scoped && scoped.conversation ? scoped.conversation : null
					} catch { conversation = null }
				}
				if (!conversation && rootCtx) {
					try { conversation = rootCtx.conversation || null } catch { conversation = null }
				}
