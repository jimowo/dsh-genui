...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const { createElement, useState, useEffect, useRef, useContext, createContext, Component, Fragment } = require("react");
		...
		const CSS = [
			...
			'.genui-field{margin:4px 0;}',
			'.genui-field-l{font-size:11px;opacity:.65;margin-bottom:2px;display:block;}',
			'.genui-input,.genui-select,.genui-textarea{font-size:12px;padding:4px 8px;border:1px solid rgba(128,128,128,.4);border-radius:6px;background:transparent;color:inherit;font-family:inherit;}',
			'.genui-input,.genui-select{width:220px;}',
			'.genui-textarea{width:100%;max-width:480px;min-height:56px;resize:vertical;}',
			'.genui-form-actions{margin-top:6px;}',,
			...
			'.genui-hbars{margin:6px 0;}',
			'.genui-hbar{display:flex;align-items:center;gap:6px;margin:3px 0;}',
			'.genui-hbar-label{font-size:10px;opacity:.6;min-width:48px;text-align:right;flex-shrink:0;}',
			'.genui-hbar-track{flex:1;height:14px;display:flex;align-items:center;}',
			'.genui-hbar-v{height:100%;border-radius:0 4px 4px 0;background:var(--ds-color-accent,#4f8ef7);opacity:.85;}',
			'.genui-hbar-num{font-size:10px;opacity:.65;margin-left:6px;}',
			'.genui-line{position:relative;height:88px;margin:6px 0;display:flex;align-items:flex-end;gap:6px;}',
			'.genui-line-svg{position:absolute;inset:0;width:100%;height:88px;pointer-events:none;}',
			'.genui-line-cols{display:flex;flex:1;align-items:flex-end;height:100%;}',
			'.genui-line-dot{width:7px;height:7px;border-radius:50%;background:var(--ds-color-accent,#4f8ef7);position:relative;top:-4px;}',
			'.genui-pie{display:flex;align-items:center;gap:14px;margin:6px 0;}',
			'.genui-pie-legend{font-size:11px;line-height:1.7;}',
			'.genui-pie-swatch{display:inline-block;width:9px;height:9px;border-radius:2px;margin-right:5px;vertical-align:baseline;}',
			'.genui-spark{display:flex;align-items:flex-end;gap:1px;height:16px;margin-top:3px;}',
			'.genui-spark i{flex:1;background:var(--ds-color-accent,#4f8ef7);opacity:.6;border-radius:1px;min-width:1px;}',
		...
		function makeSend(props) {
			return async function send(text) {
				...
				// Route through the session binding face directly
				// (sessions.binding(id).session.prompt) — the conversation service's
				// scope-addressed send keeps failing across inject boundaries
				// ("cannot get property without inject" / "requires a session scope").
				if (!sid || !rootCtx || !rootCtx.sessions) {
					throw new Error('无可用的会话发送通道（缺少 sessionId 或 sessions 服务）')
				}
				let session = null
				try {
					const binding = rootCtx.sessions.binding(sid)
					session = binding && binding.session ? binding.session : null
				} catch { session = null }
				if (!session || typeof session.prompt !== 'function') {
					throw new Error('会话 ' + sid + ' 无法寻址（无 binding）')
				}
				const result = await session.prompt([{ type: 'text', text }], 'queue')
				if (!result || result.ok !== true) {
					const err = result && result.error ? (result.error.code + ': ' + result.error.message) : 'unknown'
		...
		function renderNode(node, depth, env) {
			...
			if (type === 'stat') {
				...
				const trend = Array.isArray(node.trend)
					? node.trend.map(v => Math.max(0, Number(v) || 0)).slice(0, 24)
					: null
				return createElement('div', { className: 'genui-stat' },
					...
					trend && trend.length > 1 ? createElement('div', { className: 'genui-spark', 'aria-hidden': true },
						trend.map((v, i) => {
							const tmax = Math.max(...trend, 0)
							return createElement('i', { key: 's' + i, style: { height: (tmax > 0 ? Math.max(8, Math.round((v / tmax) * 100)) : 8) + '%' } })
						})) : null,
					delta !== null
			...
			if (type === 'chart') {
				return createElement(Chart, { node })
			}
			if (type === 'collapse') {
			...
			if (type === 'input' || type === 'select' || type === 'textarea') {
				return createElement(Field, { node })
			}
			if (type === 'form') {
				return createElement(Form, { node, depth, env })
			}
			if (type === 'text') {
		...
		/** v0.5 chart kinds. PIE_PALETTE cycles for un-colored pie slices. */
		const PIE_PALETTE = ['#4f8ef7', '#2e9e5b', '#d09b2d', '#d05050', '#8e6fd0', '#3ab0b0', '#e078ad', '#8a9aa8']

		/**
		 * Chart: kind bar (default) | line | pie | horizontal. color overrides
		 * the bar/line accent (CSS color string); items may carry per-item color
		 * (pie/horizontal). Values coerce via Number()||0; caps stay 24 items.
		 */
		function Chart({ node }) {
			const src = Array.isArray(node.items) ? node.items.slice(0, 24) : []
			const items = src.map(item => (item && typeof item === 'object' ? {
				label: text(item.label),
				value: Number(item.value) || 0,
				color: typeof item.color === 'string' ? item.color : '',
			} : { label: '', value: 0, color: '' })).filter(it => it.label !== '' || it.value > 0)
			const kind = ['bar', 'line', 'pie', 'horizontal'].includes(text(node.kind)) ? text(node.kind) : 'bar'
			const accent = typeof node.color === 'string' && node.color ? node.color : ''

			if (items.length === 0) return null
			const nums = items.map(it => it.value)
			...
			if (kind === 'pie') {
				...
				const slices = items.map((it, i) => {
					...
					const a0 = angle
					angle += frac * 360
					return { ...it, a0, a1: angle, frac, color: it.color || PIE_PALETTE[i % PIE_PALETTE.length] }
				})
				const arc = (cx, cy, r, s) => {
					const p = a => [cx + r * Math.cos(a * Math.PI / 180), cy + r * Math.sin(a * Math.PI / 180)]
					const [x0, y0] = p(s.a0), [x1, y1] = p(s.a1)
					const large = s.a1 - s.a0 > 180 ? 1 : 0
					return 'M ' + cx + ' ' + cy + ' L ' + x0 + ' ' + y0 + ' A ' + r + ' ' + r + ' 0 ' + large + ' 1 ' + x1 + ' ' + y1 + ' Z'
				}
				return createElement('div', { className: 'genui-pie' },
					createElement('svg', { width: 96, height: 96, viewBox: '0 0 96 96' },
						slices.map((s, i) => createElement('path', {
							key: 'p' + i, d: arc(48, 48, 44, s), fill: s.color,
							stroke: 'rgba(128,128,128,.25)', strokeWidth: 1,
						}, createElement('title', null, s.label + ': ' + s.value + ' (' + Math.round(s.frac * 100) + '%)')))),
					createElement('div', { className: 'genui-pie-legend' },
						slices.map((s, i) => createElement('div', { key: 'l' + i },
							createElement('span', { className: 'genui-pie-swatch', style: { background: s.color } }),
							s.label + ' — ' + s.value + ' (' + (total > 0 ? Math.round(s.frac * 100) : 0) + '%)'))))
			}

			if (kind === 'line') {
				const w = 100, h = 88, n = items.length
				const pts = items.map((it, i) => [
					n > 1 ? (i / (n - 1)) * (w - 8) + 4 : w / 2,
					h - 6 - (max > 0 ? (it.value / max) * (h - 16) : 0),
				])
				const d = pts.map((p, i) => (i === 0 ? 'M' : 'L') + p[0].toFixed(1) + ' ' + p[1].toFixed(1)).join(' ')
				return createElement('div', { className: 'genui-line' },
					createElement('svg', { className: 'genui-line-svg', viewBox: '0 0 ' + w + ' ' + h, preserveAspectRatio: 'none' },
						createElement('path', { d, fill: 'none', stroke: accent || 'var(--ds-color-accent,#4f8ef7)', strokeWidth: 2, vectorEffect: 'non-scaling-stroke' })),
					createElement('div', { className: 'genui-line-cols' },
						items.map((it, i) => createElement('div', {
							key: 'ld' + i, style: { flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'flex-end', height: '100%' },
							title: it.label + ': ' + it.value,
						},
							createElement('span', { className: 'genui-line-dot', style: accent ? { background: accent } : undefined })))),
					createElement('div', { style: { position: 'absolute', bottom: -2, left: 0, right: 0, display: 'flex' } },
						items.map((it, i) => createElement('div', { key: 'll' + i, style: { flex: 1, textAlign: 'center', fontSize: 10, opacity: .6 } }, it.label))))
			}

			if (kind === 'horizontal') {
				return createElement('div', { className: 'genui-hbars' },
					items.map((it, i) => createElement('div', { className: 'genui-hbar', key: 'hb' + i },
						createElement('span', { className: 'genui-hbar-label', title: it.label }, it.label),
						createElement('div', { className: 'genui-hbar-track' },
							createElement('div', { className: 'genui-hbar-v', style: { width: (max > 0 ? Math.round((it.value / max) * 100) : 0) + '%', background: it.color || accent || undefined } }),
							createElement('span', { className: 'genui-hbar-num' }, it.value)))))
			}

			// kind === 'bar' (default)
			return createElement('div', { className: 'genui-bars' }, items.map((it, index) => {
				const height = max > 0 ? Math.round((it.value / max) * 100) : 0
				return createElement('div', { className: 'genui-bar', key: 'bar' + index, title: it.label + ': ' + it.value },
					createElement('div', { className: 'genui-bar-v', style: { height: height + '%', background: it.color || accent || undefined } }),
					createElement('div', { className: 'genui-bar-l' }, it.label))
			}))
		}

		/**
		 * v0.4 form components. Fields keep their own local state; Form wraps
		 * registered fields (context registry keyed by field name), and on submit
		 * packs every value into the v0.3 event envelope payload — one user
		 * message, one API turn:
		 *
		 *   [ui_card 事件] {"cardId":"…","action":"…","payload":{"name":…,…}}
		 */
		const FormCtx = createContext(null)

		/** One field: input | select | textarea (name required; label optional). */
		function Field({ node }) {
			const reg = useContext(FormCtx)
			const name = text(node.name)
			const label = text(node.label)
			const kind = node.type === 'select' ? 'select' : node.type === 'textarea' ? 'textarea' : 'input'

			const [value, setValue] = useState(kind === 'select'
				? (Array.isArray(node.options) && node.options.length ? text(node.options[0]) : '')
				: text(node.value))
			...
			const set = v => { setValue(v); if (reg && name) reg(name, () => v) }
			const inputEl = kind === 'select'
				? createElement('select', {
					className: 'genui-select', value, key: 'i',
					onChange: e => { set(e && typeof e.target === 'object' ? e.target.value : '') },
				}, (Array.isArray(node.options) ? node.options : []).slice(0, 30).map((o, i) =>
					createElement('option', { key: 'o' + i, value: text(o) }, text(o))))
				: kind === 'textarea'
					? createElement('textarea', {
						className: 'genui-textarea', value, key: 'i',
						placeholder: text(node.placeholder) || '',
						onChange: e => { set(e && typeof e.target === 'object' ? e.target.value : '') },
					})
					: createElement('input', {
						className: 'genui-input', value, key: 'i',
						type: node.password === true || node.inputType === 'password' ? 'password' : 'text',
						placeholder: text(node.placeholder) || '',
						onChange: e => { set(e && typeof e.target === 'object' ? e.target.value : '') },
					})

			return createElement('div', { className: 'genui-field' },
				label ? createElement('label', { className: 'genui-field-l' }, label + (name ? ' (' + name + ')' : '')) : null,
				inputEl)
		}

		/** Form: children rendered inside a field registry; submit button packs
		 *  all registered values into the envelope payload (confirm-then-send). */
		function Form({ node, depth, env }) {
			const fieldsRef = useRef({})
			const reg = (name, getter) => { fieldsRef.current[name] = getter }
			const [phase, setPhase] = useState('') // '' | 'confirm' | 'sent' | 'error'
			const [error, setError] = useState('')
			const send = env && typeof env.send === 'function' ? env.send : null
			const action = text(node.action)
			const submitLabel = text(node.submitLabel) || '提交'

			function collect() {
				const out = {}
				for (const [k, getter] of Object.entries(fieldsRef.current)) {
					try { out[k] = getter() } catch { out[k] = '' }
				}
				return out
			}

			function doSubmit() {
				if (!send) { setPhase('error'); setError('无会话作用域，无法提交'); return }
				...
				setPhase('pending-send')
				send(envelope + '\n（界面表单提交，请按 action 与 payload 内容处理）')
					.then(() => { setPhase('sent'); setError('') })
					.catch(e => { setPhase('error'); setError('发送失败: ' + (e && e.message ? e.message : String(e))) })
			}

			function click() {
				if (phase === 'sent' || phase === 'pending-send') return
				if (node.confirm === false) { doSubmit(); return }
				if (phase === 'confirm') doSubmit()
				else setPhase('confirm')
			}

			let cls = 'genui-btn', label = submitLabel
			if (phase === 'confirm') { cls += ' genui-btn-pending'; label = submitLabel + ' — 确认提交？' }
			if (phase === 'sent') { cls += ' genui-btn-sent'; label = '✓ 已提交' }
			if (phase === 'pending-send') { cls += ' genui-btn-pending'; label = submitLabel + ' — 发送中…' }

			const kids = Array.isArray(node.children) ? node.children : (node.children ? [node.children] : [])
			return createElement(FormCtx.Provider, { value: reg },
				createElement('div', { className: 'genui-form' },
					kids.slice(0, 30).map((child, index) =>
						createElement(Fragment, { key: 'fc' + index }, renderNode(child, depth + 1, env))),
					createElement('div', { className: 'genui-form-actions' },
						createElement('button', { className: cls, onClick: () => { click() } }, label),
						error ? createElement('span', { className: 'genui-note', style: { color: '#d05050', marginLeft: 8 } }, error) : null)))
		}
		...
		/**
		 * Render error boundary: a component crash inside the card tree (like
		 * v0.4's raw-spec-children bug) degrades to a note card instead of a
		 * blank/disappeared card or a GUI-level crash.
		 */
		class ErrorBoundary extends Component {
			...
			render() {
				if (this.state.error) {
					return createElement('div', { className: 'genui-card genui-note', style: { color: '#d05050' } },
						'GenUI: 卡片渲染出错已降级 — ',
		...
		function GenUICard(props) {
			...
			return createElement('div', { className: 'genui-root' },
				createElement(ErrorBoundary, null, el),
				diag.el())
		}
